...
typedef ControlsWidgetBuilder = Widget Function(BuildContext context, ControlsDetailsX details);
...
class StepX {
  ...
  final StepStateX state;

  /// Whether or not the step is active. The flag only influences styling.
  final bool isActive;

  /// Only [StepperTypeX.horizontal], Optional widget that appears under the [title].
...
class StepperCustom extends StatefulWidget {
  ...
  /// [StepperTypeX.horizontal], the content of the current step is displayed
  /// underneath as opposed to the [StepperTypeX.vertical] case where it is
  ...
  @override
  State<StepperCustom> createState() => _StepperState();
}
  ...
  Widget _buildCircle(int index, bool oldState,int current) {
    ...
    return Container(
      ...
      child: AnimatedContainer(
        ...
        child: Center(
          child: _buildCircleChild(index, oldState && widget.steps[index].state == StepStateX.error),
        ),
      ),
    );
  }
  ...
  Widget _buildIcon(int index,int current) {
    if (widget.steps[index].state != _oldStates[index]) {
      return AnimatedCrossFade(
        ...
        crossFadeState: widget.steps[index].state == StepStateX.error ? CrossFadeState.showSecond : CrossFadeState.showFirst,
        duration: kThemeAnimationDuration,
      );
    } else {
  ...
  Widget _buildHorizontal(int isCurrent) {
    final List<Widget> children = <Widget>[      
      for (int i = 0; i < widget.steps.length; i += 1) ...<Widget>[
        InkResponse(
          onTap: widget.steps[i].state != StepState.disabled ? () { widget.onStepTapped?.call(i); } : null,
